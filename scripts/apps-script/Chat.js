...
function handleChat_(data) {
  ...
  var trace = [];
  var raw = callModel_(key, messages, props.getProperty('NVIDIA_MODEL'), trace);
  if (raw === null) {
    var out = { ok: false, reply: 'I could not reach the assistant just now — WhatsApp is the fastest route.' };
    // Only ever returned when explicitly asked for, so a failing endpoint
    // does not narrate its internals to whoever happens to POST at it.
    if (data.debug === true) out.trace = trace;
    return out;
  }
...
function callModel_(key, messages, pinned, trace) {
  ...
  for (var i = 0; i < models.length; i++) {
    ...
    try {
      ...
      if (code !== 200) {
        trace.push(model + ' -> HTTP ' + code + ' ' + res.getContentText().slice(0, 120));
        continue;
      }
      ...
      trace.push(model + ' -> 200 but empty content');
    } catch (err) {
      // Try the next model rather than failing the guest. The reason is kept
      // so a failure can be diagnosed without another deploy cycle.
      trace.push(model + ' -> threw: ' + String(err).slice(0, 140));
    }
  }
  return null;
}
