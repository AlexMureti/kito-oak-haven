...
/**
 * Brings an older sheet up to the current column layout.
 *
 * Headers are only written when the sheet is empty, so a sheet created before
 * Check-in, Check-out and Guest name existed keeps its ten-column row forever
 * while the code appends thirteen values — every field after "Came from" lands
 * one column left of where it belongs, silently, on live bookings.
 *
 * Inserting columns rather than rewriting the header row keeps the data that
 * is already there aligned with it. Guarded on the exact old shape so it
 * cannot run twice or fire on a layout it does not recognise.
 */
function migrateHeaders_(sheet) {
  ...
  if (String(head[7]) !== 'Came from' || String(head[8]) !== 'Status') return;

  sheet.insertColumnsAfter(8, 3);
  sheet.getRange(1, 9, 1, 3)
       .setValues([['Check-in', 'Check-out', 'Guest name']])
       .setFontWeight('bold')
